...
nested_array(Depth) ->
    iolist_to_binary([lists:duplicate(Depth, $[), lists:duplicate(Depth, $])]).

%% A nested object of the given depth, e.g. depth 2 -> <<"{\"a\":{\"a\":1}}">>.
nested_object(1) ->
    <<"{\"a\":1}">>;
nested_object(Depth) ->
    iolist_to_binary([<<"{\"a\":">>, nested_object(Depth - 1), <<"}">>]).
...
deeply_nested_input_rejected_test() ->
    %% A pathologically deep input must bail out via the depth guard as soon
    %% as the limit is crossed, rather than exhausting the C stack or the
    %% allocator first. The guard fires regardless of how much deeper it goes.
    Json = nested_array(100000),
    ?assertError({_, max_depth_exceeded}, dec(Json, [{max_depth, 256}])).
